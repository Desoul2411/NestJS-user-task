...
describe('UsersController', () => {
  ...
  const updatedUserExpectedResult = {
    id: 1,
    userNameHashed:
      'b8ea7132aeb096a3cdc8cd453075f8bd53ead6120baf545299e4949aeb9ff5bc',
    userPasswordEncrypted: passwordGenerated,
    group: 1,
  };
  ...
  beforeEach(async () => {
    ...
    /*  userDomainService = moduleRef.get<UpdateUserUseCase>(
  ...
  describe('update', () => {
    ...
    it('should return updated user object - success', async () => {
      ...
      expect(await usersController.update(1, createUserDataDto)).toEqual(
    ...
    it('should throw an error with status 403 and message "Invalid old password!" - fail', async () => {
      updateUserMock.mockRejectedValue(
        new HttpException('Invalid old password!', HttpStatus.FORBIDDEN),
      );
