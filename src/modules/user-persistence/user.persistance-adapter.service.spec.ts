...
describe('UserPersistenceAdapterService', () => {
  ...
  let nameHashedGenerated,
    userPasswordEncryptedGenerated,
  ...
  beforeEach(async () => {
    ...
    userPersistenceAdapterService = moduleRef.get<UserPersistenceAdapterService>(
      UserPersistenceAdapterService,
    );
    ...
    (cipherUtils.hashToSha256 as jest.Mock).mockImplementation(
      () => nameHashedGenerated,
    );
    (cipherUtils.encrypt as jest.Mock).mockImplementation(
      () => userPasswordEncryptedGenerated,
    );
  ...
  describe('createUser', () => {
    it('should call the repository with correct paramaters and return created user object', async () => {
      ...
      const userRepositorySaveSpy = jest
        ...
        .mockResolvedValue(createdUserExpectedResult);

      const res = await userPersistenceAdapterService.createUser(
        createUserDataDto,
      );
  ...
  describe('loadUserByName', () => {
    it(`call the repository and mapUserByNametoDomain function with correct paramaters 
      ...
      const mapUserByNametoDomainSpy = jest
        .spyOn(UserMapper, 'mapUserByNametoDomain')
        .mockReturnValue(mappedToDomainEntityUser);

      const res = await userPersistenceAdapterService.loadUserByName(
        createUserDataDto.userName,
      );
  ...
  describe('loadUserById ', () => {
    it('call the repository and mapUserByNametoDomain function with correct paramaters and return UserEntity instance', async () => {
      ...
      (cipherUtils.decrypt as jest.Mock).mockImplementation(
        () => currentUserDecryptedPasword,
      );
      (cipherUtils.encrypt as jest.Mock).mockImplementation(
        () => userNewPasswordEncrypted,
      ...
      const mapUserByIdtoDomainSpy = jest
        .spyOn(UserMapper, 'mapUserByIdtoDomain')
        .mockReturnValue(mappedToDomainEntityUser);
      const res = await userPersistenceAdapterService.loadUserById(
        1,
        createUserDataDto,
      );

      expect(userRepositoryFindOneSpy).toHaveBeenCalledWith(foundUser.id);
      expect(userRepositoryFindOneSpy).toHaveBeenCalledTimes(1);
    ...
    it('should throw NOT_FOUND error with status 404 and "No such user" error message', async () => {
      jest
        .spyOn(userRepository, 'findOne')
        .mockRejectedValue(
          new HttpException('No such user', HttpStatus.NOT_FOUND),
        );
  ...
  describe('updateUserState', () => {
    it('should call the repository with correct parameters and return updated user', async () => {
      ...
      const mapToUserOrmEntitySpy = jest
        .spyOn(UserMapper, 'mapToUserOrmEntity')
        .mockReturnValue(updatedUser);
      const userRepositorySaveSpy = jest
        .spyOn(userRepository, 'save')
        .mockResolvedValue(updatedUser);

      const res = await userPersistenceAdapterService.updateUserState(
        domainUser,
      );
