...
import {
  ...
  decrypt,
  hashToSha256,
} from '../utils/functions-helpers/cipher.utils';
...
@Injectable()
export class UserPersistenceAdapterService
  implements LoadUserByIdPort, LoadUserByNamePort {
  // must implement port from ports/out/..
  constructor(
  ...
  async loadUserById(
    ...
    const currentUserDecryptedPasword = decrypt(
  ...
  async updateUserState(user: UserEntity): Promise<UserOrmEntity> {
    try {
      return await this._userRepository.save(
        UserMapper.mapToUserOrmEntity(user),
      );
    } catch (error) {
      throw new HttpException(
        'INTERNAL_SERVER_ERROR',
