...
export class UpdateUserService implements UpdateUserUseCase {
  ...
  async updateUser(command: UpdateUserCommand) {
    const userById: UserEntity = await this._loadUserByIdPort.loadUserById(
      command.id,
      command.userData,
    );
    const userByName: UserEntity = await this._loadUserByNamePort.loadUserByName(
      command.userData.userName,
    );

    if (
      !userById.comparePasswords(
