...
describe('UsersController (e2e)', () => {
  ...
  const expectedCreatedUserBody = {
    userNameHashed:
      'a6a73e1a9fd6a391c628c7ddae17acae5a79a2ebb6ea820c2c004de18ecc3cb7',
    userPasswordEncrypted:
      'SnAzcW40MEJhaHhYSmxwVkoxRkh5TW9IQWhHK0ZuRlcyL284bHBxY3kxYz0tLTJGN3kwT3p0MUlaei9Ma2VnVS9hWVE9PQ==',
    group: null,
    id: 1,
  };
  ...
  const expectedUpdatedUserBody = {
    id: 1,
    userNameHashed:
      'c46a79ca5792fbf3c68ce6bd1810200dfd8926e889fa375ce7efd9620dc5ac6f',
    userPasswordEncrypted:
      'cGhnRmwwWC8zSzR4VEo3aU85cjlmWkJqUjl6ZytmSmIzWUcvVTI5ZGxvST0tLUc4T2pOa0EzZGZSKzZGMTlzM2JHb0E9PQ==',
    group: 1,
  };
  ...
  it('/user (POST) - create - success', async (done) => {
    return await request(app.getHttpServer())
      ...
      .then(({ body }: request.Response) => {
        userId = body.id;
        expect(body.userNameHashed).toEqual(
          expectedCreatedUserBody.userNameHashed,
        );
  ...
  it('/user (POST) - create - fail (response status 400 with validation error message when dto properties have invalid type)', () => {
  ...
  it('/user (POST) - create - fail (response status 400 with validation error message when dto passed without required properties)', () => {
    return request(app.getHttpServer())
      .post('/user')
  ...
  it('/user (POST) - create - fail (response status 403 with validation error message - invalid signature)', () => {
    return request(app.getHttpServer())
      .post('/user')
  ...
  it('/user (PUT) - update - success', async (done) => {
    return request(app.getHttpServer())
      ...
      .then(({ body }: request.Response) => {
        ...
        expect(body.userNameHashed).toEqual(
          expectedUpdatedUserBody.userNameHashed,
        );
  ...
  it('/user (PUT) - update - fail (response status 400 - dto properties have invalid type)', () => {
    return request(app.getHttpServer())
      .put('/user/1')
  ...
  it('/user (PUT) - update - fail  (response status 400 - dto passed without required properties)', () => {
    return request(app.getHttpServer())
      .put('/user/1')
